...
## 1.1 在 Codex / Claude Code 中使用

推荐让 agent 通过 MCP 直接调用 Agent Team，而不是手动解析 YAML。

### Codex

1. 将 [.codex/config.agent-team.example.toml](/Users/gegarron/Documents/AI Agent/.codex/config.agent-team.example.toml) 的内容复制到可信项目的 `.codex/config.toml` 或 `~/.codex/config.toml`。
2. 重启 Codex。
3. 使用 `$agent-team` 或自然语言触发：

```text
$agent-team 用 new-product-mvp 规划一个 AI 记账工具 MVP
```

Codex Skill 位于 [.agents/skills/agent-team/SKILL.md](/Users/gegarron/Documents/AI Agent/.agents/skills/agent-team/SKILL.md)。

### Claude Code

1. 参考 [.mcp.example.json](/Users/gegarron/Documents/AI Agent/.mcp.example.json) 配置本地 stdio MCP server。
2. 使用 `/agent-team` 触发：
...
Claude Code Skill 位于 [.claude/skills/agent-team/SKILL.md](/Users/gegarron/Documents/AI Agent/.claude/skills/agent-team/SKILL.md)。

### MCP 工具

MCP server 通过 `node src/agent-team/cli.js mcp` 启动，暴露 workflow 查询、run 创建、step prompt、审批、输出提交、事件查看和取消 run 等工具。所有运行状态仍写入 `.agent-team/runtime`，Web Workbench 可以继续查看这些 run。
...
Division 是能力域。当前完整 registry 有 15 个 Division，常用能力域如下：
